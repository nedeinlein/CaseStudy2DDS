...
#Base data import
base <- read.csv('https://raw.githubusercontent.com/nedeinlein/CaseStudy2DDS/main/CaseStudy2data.csv')
...
## Modeling KNN
After looking at the data visually, the first model I wanted to test was a simple KNN model. Because many of the variables did not appear to have stark differences between what might cause attrition and what would not, I worked on an original model with all continuous variables and subtracted until I came up with the final model. For this reason the initial plot was to try to find a base line K value to start with in the model.
```{r #KNN test of model}
...
#Improving Accuracy
After running this, the most accurate model that the sum of the data could create with all variables was at K = 11. This put the sensitivity level at 60%, Specificity at around 85%, and accuracy at 85%. To improve the model, I then started removing variables. Variables that caused a large drop in accuracy were kept and at each run, the model was then checked to see if the drop was due to the variable or if it was simply an issue of needing to change the K value to something other than 11.
...
#Final Model KNN
After pushing through the process of adding and removing variables, I came up with the final model. The final three factors that created the most accurate model and were thus most influentian to attrition in this model were DailyRate, MonthlyIncome, and MonthlyRate. However, as you can see from the statistics, they are identical to the initial KNN model. This would indicate these are the main three factors that are influencing the model. 
...
##NaiveBayes
While the statistics from the KNN were not completely useless, the model struggled with sensitivity. For this reason, I tested NaiveBayes as a possible alternative to the simple KNN model. My first approach was to plot all variables to get a baseline metric. I then plotted the strongest indicators from the KNN model to see if this would render a more accurate model than the baseline.
...
After running the strongest indicators from the KNN model and the sensitivity metric coming back as 0, I went back to starting with all variables then removing variables that would improve or keep the metrics the same and keeping those that reduced the metrics of the model. This resulted in the following findings:
...
When comparing the final models for both KNN and NaiveBayes, NaiveBayes was very clearly the more appropriate model for this data set and what was attempting to be predicted. For this reason it was used to predict Attrition based on a test set supplied by the "client". Below is the code for exporting the predictions as directed. These have been uploaded to the github repository for this case study.
...
write.csv(Predictions,file = "Case2PredictionsDeinlein Attrition.csv")
```
...
#Final Regression
After porting the adjusted R-Squared values into the easy to read table, I determined that JobLevel, JobRole, and TotalWorkingYears, would probably be my final model. But first, I tested to make sure that no more accurate model could be created than just using those those three variables. This was done by substituting variables and removing variables. But this ended up being the final model based on the RMSE.
...
The above model was then used on a test set supplied by the "client" to predict salary. Below is the code that was used to run the prediction and then export the data in the appropriate format for submission.
...
write.csv(Predictionsalary,file = "Case2PredictionsDeinlein Salary.csv")
```